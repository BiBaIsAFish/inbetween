...
function draw(cards) { // draw one card and push to cards
    ...
    while (drawn) {
        ...
        if (i != cards.length) {// one of the cards has been drawn
...
function printCard(card) { // number to card
    let result = "";
...
function checkResult1(cards) {// for check inside or outside
    let left = cards[0] % 13;
    let right = cards[2] % 13;
    let between = cards[1] % 13;
    if (left == between || right == between) {
        return 0; // hit on the door
    } else if ((left < between && between < right) || (left > between && between > right)) {
        return 1;
    } else {
        return -1;
    }
...
function checkResult2(cards, flag) {// for guess above or below
    let left = cards[0] % 13;
    let between = cards[1] % 13;

    if (left == between) {// A A A
        return 0;
    } else if ((flag == -1 && left - between > 0) || (flag == 1 && between - left > 0)) { // A 10 A or K 5 K
        return 1;
    } else { // 6 5 6 or 10 K 10
        return -1;
    }
}
...
function play() {
    ...
    if (checkDoor(cards) == 0) { // if guess above or below; print rule at console
...
function showCard(index) { // update target card; 0:left 1:middle 2:right
...
function open1(bet) { // open middle card; in guessing inside or outside
...
function count(limit) { // animation for bet number
...
confirmButton.addEventListener('click', (e) => { //listen a button; click to start game
    e.preventDefault();
...
bigButton.addEventListener('click', () => { // guess big and open the result
...
openButton.addEventListener('click', () => { // open the result of inside or outside
    if (canClickOPEN) {
        let bet = document.getElementById('bet-input').value;
